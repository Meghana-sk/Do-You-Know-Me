...
console.log("*****************************************************")
let score = 0
let Leaderboard = [{name : "Nithin",Score :10},
                  {name : "Deepika",Score : 8},
                  {name : "Sheetal",Score : 7}]

const play = (question,answer,explanation) => {
...
let questionnaire = [QuestionOne,QuestionTwo,QuestionThree,QuestionFour,QuestionFive,QuestionSix]
...
let currentPlayer = {name : userName, Score : score}
Leaderboard.push(currentPlayer);
let temp = {};
for(let j = 0; j < Leaderboard.length - 1; j++){
  for(let k = j+1;k <  Leaderboard.length; k++){
